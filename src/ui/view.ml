...
let display_timeline events title =
  let events = List.map (fun (_, e) -> e) events in
  let title = match title with None -> None | Some (_, t) -> Some t in
  let cmd =
    ...
    Format.asprintf
      "window.timeline = new TL.Timeline('page-content',%s)"
      str in
...
let init_slide_from_url res = begin 
    match res with
    | Error e ->
...
let make_page events title = let open Lwt in
  (display_timeline events title >>= init_slide_from_url) >>=
  (fun _ -> Lwt.return (Ok ())) |> ignore
