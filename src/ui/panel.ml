...
module PageInput = struct
  ...
  let get_page id page page_number =
    let page_input = find_component @@ Ui_utils.make_id "input" id in
    let curr =
  ...
  let mk_popover id page page_number goto =
    div ~a: [ a_class [ input_group; input_group_sm ] ] [
      ...
      input ~a:[ a_input_type `Text; a_class [ form_control; "page-input" ];
                 a_placeholder "Go to"; a_id @@ Ui_utils.make_id "input" id;
               ] ();
      span ~a:[ a_class [ input_group_btn ] ] [
  ...
  let init_inputs ids page page_number goto =
    ...
    List.iter (fun id ->
        let j_obj =
          Printf.sprintf "jQuery(\'#%s\')" (Ui_utils.make_id "link" id) in
  ...
  let link_with_input ?(prefix="") =
    let create_id = let cpt = ref 0 in fun () ->
        incr cpt;
        Ui_utils.make_id (prefix ^ "goto-page") (string_of_int !cpt) in
    fun value ->
      let id = create_id () in
...
let large_page_range page page_total =
  ...
  | Some page_total ->
    if page_total <= 5
    then Utils.list_init page_total make_page
...
let find_url_arg arg =
  List.find_opt (fun (k, _v) -> k = arg) Url.Current.arguments

let set_page_in_browser_url ?(urlarg="p") page =
  if urlarg <> "" then
    Ui_utils.set_url_arg ~default:"1" urlarg (string_of_int page)

let set_size_in_browser_url ?(urlarg="r") page =
  if urlarg <> "" then
    Ui_utils.set_url_arg ~default:"20" urlarg (string_of_int page)
        ...
        div [
          div ~a:[ a_class [ row ] ] [
            ...
            div ~a:[  a_id loading_id ]
              [ Ui_utils.make_loading_gif [ "loading-heading"; clg8; csm6; cxs6 ] ]
          ]
        ])
      ~panel_body_content:[
        div ~a: [ a_id table_id ] [
          tablex ~a:[ a_class [ btable; M.table_class ] ] [
            tbody
              (theads() :: [
                  tr [
                    td [
                      Jslang.txt_t @@ ss_ "Fetching data" ] ] ] )]
        ]
      ] ()
          ...
          div [
            div ~a:[ a_class [ row ] ] [
              ...
              div ~a:[ a_id (loading_id ^ suf_id) ; a_class [ clg8; cxs7; "paginate"] ]
                [Ui_utils.make_loading_gif [ "loading-heading" ] ]
            ]
          ])
        ~panel_body_content:
          ([ div
                ...
                div ~a:[ a_class [ panel_heading ] ] [
                      div [
                            h3 ~a:[ a_id title_id;
                               ...
                               [M.title_span (-1) ];
                            div ~a:[ a_id loading_id ]
                                [Ui_utils.span_loading_gif [ "loading-heading";
                                                           clg4]]
                    ]
                    ] ;
    ...
    let rec update_panel page page_size (rows: M.data) =
      ...
      let mk_onclick_size new_page_size id =
        (fun _ ->
            ...
            elt##.innerHTML :=
              Js.string (Printf.sprintf "%s: %d" (t_ @@ ss_ "Rows") page_size);
      ...
      let make_page_sizer () =
        if not page_sizer then span ~a:[a_class ["hidden"]] []
          ...
          bootstrap_dropdown_button
            ~btn_class:[ btn_secondary; btn_sm; ]
            ~ctn_class:["hidden-xs"; "page-size"]
            id
            [ txt (Printf.sprintf "%s: %d" (t_ @@ ss_ "Rows") page_size) ]
            select
      in
...
let div_maker data =
  match data with
  | Loading table_id ->
     div ~a:[ a_id table_id ] [Jslang.txt_t @@ ss_ "Fetching data" ]
...
module MakePageTableList(
  ...
  let make_page_sizer page_sizer page page_size f =
    let mk_onclick_size new_page_size id _ =
      let b = find_component id in
      let elt = To_dom.of_select b in
      elt##.innerHTML := Js.string (Printf.sprintf "%s: %d" (t_ @@ ss_ "Rows") page_size);
      let new_page = page * page_size / new_page_size in
